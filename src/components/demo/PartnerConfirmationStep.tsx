...
interface PartnerConfirmationStepProps {
  onBack?: () => void;
  onNext?: () => void;
  embedded?: boolean;
}

export const PartnerConfirmationStep = ({
  onBack,
  onNext,
  embedded = false,
}: PartnerConfirmationStepProps) => {
  const [sent, setSent] = useState(false);

  const stepLabel = embedded ? "Rollivaade — Partner" : "02 — Partneri kinnitus";
  const title = embedded ? "Partneri vaade" : "Partneri kinnitus";
  const subtitle = embedded
    ? "Spordikool või huvikool kinnitab õppija tegevuse — tõendatud sisendina kooli otsustamiseks."
    : "Spordikool kinnitab treeningute sageduse, võistlused, laagrid ja arendatud oskused.";

  return (
    <DemoShell stepLabel={stepLabel} title={title} subtitle={subtitle}>
      {!embedded && (
        <ParentNote text="Spordikooli treener või huvikooli juhendaja kinnitab, mida õppija on tegelikult teinud: kui tihti, kui kaua ja milliseid oskusi arendanud. Partner ei pane hinnet ega tee otsust kooli eest — ta annab tõendatud sisendi." />
      )}
      ...
      <div className="rounded-2xl border border-border bg-muted/30 p-5 flex items-center gap-4 mb-7">
        ...
        <div className="min-w-0">
          ...
          <div className="text-base font-semibold tracking-tight">Pärnu Spordikool</div>
      ...
      <div className="mt-8 pt-7 border-t border-border flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <Button
            ...
            disabled={sent}
            className="shadow-elevated group flex-1 md:flex-none"
          >
            {sent ? (
              <>
                <CheckCircle2 className="size-4" />
